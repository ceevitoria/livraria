...
import com.cee.livraria.persistence.jpa.relatorio.RelatorioVendaPeriodoDAO;
...
@SPlcRepository
@PlcAggregationIoC(clazz = RelatorioVendaPeriodo.class)
public class RelatorioVendaPeriodoRepository extends PlcBaseRepository {
	...
	@Inject
	RelatorioVendaPeriodoDAO relatorioDAO;
	...
	public Collection recuperaVendaPeriodo(PlcBaseContextVO context, RelatorioVendaPeriodo relatorioArg, String orderByDinamico, Integer inicio, Integer total) throws PlcException {
		...
		List<RelatorioVendaPeriodo> vendasPeriodo = relatorioDAO.findList(context, orderByDinamico, inicio, total, relatorioArg.getDataInicio(), relatorioArg.getDataFim());
	...
	public byte[] gerarRelatorioVendaPeriodo(PlcBaseContextVO context, RelatorioVendaPeriodo relatorioArg, String sheetName) throws PlcException  {
		...
		List<RelatorioVendaPeriodo> vendasPeriodo = (List<RelatorioVendaPeriodo>) recuperaVendaPeriodo(context, relatorioArg, null, null, null);
		...
		for (RelatorioVendaPeriodo vendaPeriodo: vendasPeriodo) {
			row = sheet.createRow(linhaExcel++);
			populaDadosVendaPeriodo(vendaPeriodo, styles, factory, wb, sheet, row);
		
			quantidadeTotal += vendaPeriodo.getQuantidade();
			valorTotal = valorTotal.add(vendaPeriodo.getValorTotal());
		}		
		...
		try {
			...
			throw new PlcException("RelatorioVendaPeriodoRepository", "gerarRelatorioVendaPeriodo", e, log, "");
		}

	}

	private void configuraRodape(Map<String, XSSFCellStyle> styles, XSSFSheet sheet, XSSFRow row, Long quantidadeTotal, BigDecimal valorTotal) {
		XSSFCell cell;
	...
	private void configuraTitulo(Map<String, XSSFCellStyle> styles, CreationHelper factory, XSSFSheet sheet, RelatorioVendaPeriodo relatorioArg) {
		XSSFRow row;
		XSSFCell cell;
	...
	private void populaDadosVendaPeriodo(RelatorioVendaPeriodo vendaPeriodo, Map<String, XSSFCellStyle> styles, CreationHelper factory, XSSFWorkbook wb, XSSFSheet sheet, XSSFRow row) {
		XSSFCell cell;
		
		cell = row.createCell(COL.CODIGO_BARRAS);
		cell.setCellValue(vendaPeriodo.getProduto().getCodigoBarras());
		cell.setCellStyle(styles.get("codigoBarrasValue"));
		cell.setCellType(Cell.CELL_TYPE_STRING);  

		cell = row.createCell(COL.TITULO);
		cell.setCellValue(vendaPeriodo.getProduto().getTitulo());

		cell = row.createCell(COL.TIPO_PRODUTO);
		cell.setCellValue(vendaPeriodo.getProduto().getTipoProduto().getName());
		cell.setCellStyle(styles.get("tipoProdutoValue"));

		cell = row.createCell(COL.QUANTIDADE);
		cell.setCellValue(vendaPeriodo.getQuantidade());

		cell = row.createCell(COL.VALOR_TOTAL);
		cell.setCellValue(vendaPeriodo.getValorTotal().doubleValue());
