 ...
 * As fun��es abaixo, est�o dispostas no formato jQuery, dando suporte para o funcionamento do sistema na camada vis�o.
 */

plc = {
	/**
	 * Transforma um Form em um Objeto.
	 * @param form Seletor jQuery, ou DOM do formul�rio.
	 * @param preffix Prefixo presente nos elementos do formul�rio, que deve ser descartado.
	 * 
	 * O Prefixo � comum em JSF. 
	 */
	uriParam:'?evento=y'
	...
	,janelaModal: function(c, dialogOpener){
		...
		//verificando se j� existe uma janela modal
	...
	,criaSelecaoJqGrid: function(idGrid, idNavegacao, configuracaoGrid){
		...
		var pesquisaAjax = function(postData){
			...
			plc.jq.ajax({
				...
				,success: function(data){
					...
					// Em caso de chave natural, o objeto n�o tem ID, e o Grid precisa do ID para identificar a linha
		...
		// Sempre quando a tabela termina de ser carregada a sua altura � reajustada basedo
	...
	/**
	 * Exibe mensagem no cabe�alho da aplica��o. Se a mensagem que for enviada for nula, o cabe�alho ser� limpado
	 */
	,mensagem: function( type, msg, clear, classMsg, classIcon, container ) {
		...
		// Possibilita for�ar o container de mensagens.
			...
			if ( type === false ) {
				...
				if ( $msg.hasClass( classMsg ) ) {
					...
					if ( msg === false ) {
						...
						// mensagem(type, "msg", false) n�o limpa mensagens anteriores.
						if ( clear !== false ) {
							$msg.empty( ).hide( );
						}
						// Uma unica mensagem.
	...
	/**
	 * Defini uma tecla de atalho, correspondendo a alguma a��o do jCompany.
	 */
	,hotkey: function( hotkey, acao ) {
		...
		if(typeof(acao) == "function") {
			...
			//se o botao for null � porque n�o achou ou � um link
			if (botao == null) {
				botao = plc.jq("#" + acao);
			}	
